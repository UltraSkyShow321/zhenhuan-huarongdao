...
- 🎭 **甄嬛传主题**：36 个剧情关卡 × 3 档难度（杏花微雨、滴血验亲、纯元故衣……凤临天下），19 位角色各有专属立绘
- 👘 **角色立绘**：19 位角色专属古风 SVG 头像——甄嬛钿子红花、皇后凤冠、华妃旗头牡丹、皇上朝冠、太医官帽、宫女双髻……眉眼表情各不相同
- 🧩 **三档难度**：简单 / 中等 / 困难，通关 3 关解锁下一档，进度自动存档
- 💡 **智能提示**：内置 BFS 求解器，卡关时一键显示最优走法的下一步与距通关步数（棋盘上以箭头标注）
- 🏆 **星级评价**：步数越接近参考最优值星级越高；胜利时甄嬛驶出宫门并有台词彩蛋
- 🎨 **关卡编辑器**：自摆棋盘，BFS 实时验证可解性，生成 12 位分享码与好友互传
- 🏆 **成就系统**：12 枚成就（初入宫廷 → 六宫无匹）+ 深宫生涯统计页
- ⚡ **挑战模式**：限时限步变体玩法，全部 36 关挑战完成有专属成就
- 🎬 **演示模式**：一键自动播放当前局面的最优解，像看棋谱一样学走法
- 💾 **存档管理**：存档码导出/导入，跨设备、跨平台迁移进度
- 🎵 **背景音乐**：内置古风配乐，支持导入本地音乐文件（如原声 mp3，仅存本机不上传），右上角 🎵 随时开关
- 📱 **多端适配**：手机窄屏自动换行防溢出；桌面宽屏自动切换为「棋盘+信息」双栏布局
...
| 🐧 Linux | `.AppImage` / `.deb` | AppImage 赋权后运行；deb 直接双击安装 |
| 🤖 Android | `.apk` | 下载后允许「未知来源」安装；**鸿蒙 1-4 同样可装** |
| 📱 iOS / iPadOS | PWA 即装 | Safari 打开在线地址 → 分享 → 添加到主屏幕（全屏、离线）；原生工程接入见 [iOS 构建指南](mobile/IOS.md)（需 Mac+Xcode） |
| ⭕ 纯血鸿蒙 6.0+ | PWA 即装 + 原生工程 | 浏览器「添加到桌面」即玩；原生 HAP 用免费 DevEco Studio 构建（[指南](mobile/harmonyos/INSTALL.md)，约 5 分钟） |
...
**分享码怎么用？** 编辑器里摆好关卡 → 验证 → 「生成分享码」，得到 12 位码（如 Z9038B156GJ9）。好友在编辑器粘贴 → 「导入分享码」→ 试玩或保存。

**换手机/换浏览器怎么迁移进度？** 主菜单 → 💾 存档 → 「生成存档码」复制保存（或下载文件）；新设备粘贴 → 「导入（覆盖）」二次确认即可。
